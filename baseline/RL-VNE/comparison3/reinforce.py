...
class RL:
    ...
    def train(self, training_set):
        ...
        while iteration < self.num_epoch:
            ...
            for req in training_set:
                ...
                if req.graph['type'] == 0:
                    ...
                    # 当实验次数达到 batch size 整倍数, 累积的梯度更新一次参数
        ...
        with open(os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), "../")), 'results/loss-%s.txt' % self.num_epoch), 'w') as f:
        # with open('D:/迅雷下载/new/same/RL-VNE-master/results/loss-%s.txt' % self.num_epoch, 'w') as f:
...
#     print(os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), "../")), 'results/loss-1.txt'))